...
def deviation_strategy_action(player, hand, dealer, table):
    ...
    if hand.is_double and len(player.hands)<3:
        double_card_index = hand.cards[0].card_value[-1]
        if deviations_split_map[dealer_card][double_card_index]:
            return AO.SPLIT
        if len(deviations_split_map[dealer_card][double_card_index]) == 2:
            dev_count = deviations_split_map[dealer_card][double_card_index]
    ...
    if hand.hand_soft:
        if len(deviations_soft_hand_map[dealer_card][hand.soft_total_low]) == 2:
            ...
            action = deviations_soft_hand_map[dealer_card][hand.soft_total_low]
    else:
        if len(deviations_basic_hard_hand_map[dealer_card][hand.hard_sum]) == 2:
            ...
            action = deviations_basic_hard_hand_map[dealer_card][hand.hard_sum]
    action = check_double_down(action,hand,table,player)
    return action
